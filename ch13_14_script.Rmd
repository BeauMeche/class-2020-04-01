...
z_1 <- stan_glm(dvote ~ race + ideology, refresh = 0, 
...
q1 <- plogis(coef(z_1)[1] + coef(z_1)[4] * 4)
...
# this gives the upper bound increase for a 1 unit change in x
...
plogis(coef(z_1)[1] + coef(z_1)[4] * 4)
plogis(coef(z_1)[1] + coef(z_1)[4] * 4)
...
x$c_ideo <- x$ideology - 4 # median value is more convenient here
  
  # mean(x$ideology)

z_2 <- stan_glm(dvote ~ race + c_ideo, refresh = 0, 
                family = binomial(link = "logit"),
                data = x)

# intercept gives the logistic coefficient that allows us to calculate the probability of voting Dem for a black voter with an average ideology score
...
z_3 <- stan_glm(dvote ~ race + c_ideo + race * c_ideo, refresh = 0, 
                family = binomial(link = "logit"),
                data = x)
...
q3.2 <- coef(z_3)[1] + coef(z_3)[3] + coef(z_3)[4] + coef(z_3)[6]
...
lp1 <- posterior_linpred(object = z_3, 
                         transform = T)
